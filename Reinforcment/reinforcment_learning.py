...
def update_paths(env_name):
    ...
    log_reward_path = checkpoint_dir + "/log_reward.txt"

    # File-path for the log-file for Q-values.
    log_q_values_path = checkpoint_dir + "/log_q_values.txt"
...
def _pre_process_image(image):
    ...
    img_resized = cv2.resize(img, state_img_size_reverse)
...
class ReplayMemory:
    ...
    def reset(self):
        ...
        #self.q_values = np.zeros(shape=[self.size, self.num_actions], dtype=np.float)
        ##self.q_values_old = np.zeros(shape=[self.size, self.num_actions], dtype=np.float)
...
class NonLinearControlSignal(LinearControlSignal):
    def __init__(self, start_value, end_value, num_iterations, repeat=False, 
                 start_threshold=0.75,
                 period=10000,
                 swing=0.1):
        super(NonLinearControlSignal, self).__init__(start_value, 
                                                     end_value, 
                                                     num_iterations, 
                                                     repeat)
    ...
    def get_value(self, iteration):
        ...
        #m = iteration % self.period
        #value += m * self.swing
        
        m = (iteration % self.period) + 1
        m /= self.period
        value += value * self.swing * m
...
class EpsilonGreedy:
    ...
    def __init__(self, num_actions,
        ...
        #                                          end_value=end_value,
        #                                          repeat=repeat)
        self.epsilon_linear = NonLinearControlSignal(num_iterations=num_iterations,
                                                  start_value=start_value,
                                                  end_value=end_value,
                                                  repeat=repeat)
...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, replay_memory):
        ...
        net = tf.layers.dense(inputs=net, name='layer_fc1', units=768,
                              kernel_initializer=init, activation=activation)

        # Second fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc2', units=768,
                              kernel_initializer=init, activation=activation)

        # Third fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc3', units=768,
                              kernel_initializer=init, activation=activation)

        # Fourth fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc4', units=768,
                              kernel_initializer=init, activation=activation)
...
class Agent:
    ...
    def __init__(self, env_name, training, render=False, use_logging=True):
        ...
        if self.training:
            ...
            #self.learning_rate_control = NonLinearControlSignal(start_value=1e-3,
            #                                                 end_value=1e-5,
            #                                                 num_iterations=0.5e6,
            #                                                 repeat=False,
            #                                                 start_threshold=1,
            #                                                 period=10000,
            #                                                 swing=0.1)
